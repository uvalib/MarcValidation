...
<xsl:stylesheet xmlns="http://www.loc.gov/MARC21/slim" xmlns:marc="http://www.loc.gov/MARC21/slim"
    ...
    <xsl:choose>
      ...
      <xsl:when test="$repeat = 'NR'">
        <subfield>
               ...
               <xsl:value-of select="string-join($subfieldList//*:subfield[@code = $left], $separator)"/></subfield>
      </xsl:when>
      <!-- Repeatable subfield -->
      <xsl:otherwise>
        <subfield>
               ...
               <xsl:value-of select="$subfieldList//*:subfield[1]"/></subfield>
      </xsl:otherwise>
    </xsl:choose>
    ...
    <xsl:choose>
      <xsl:when test="string-length($thisValue) &gt; 3">
        <subfield code="{$thisSubfield}"><!-- Fix common incorrect value for English and Japanese -->
               <xsl:value-of select="replace(replace(replace(substring($thisValue, 1, 3), 'jap', 'jpn'), 'ing', 'eng'), 'end', 'eng')"/></subfield>
      ...
      <xsl:otherwise>
        <subfield code="{$thisSubfield}"><!-- Fix common errors -->
               <xsl:value-of select="replace(replace(replace(replace($thisValue, 'jap', 'jpn'), 'ing', 'eng'), 'end', 'eng'), 'rur', 'rus')"/></subfield>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
        ...
        <xsl:copy>
          ...
          <xsl:choose>
            <xsl:when test="@code = 'a'">
              <xsl:value-of select="string-join(../*:subfield[@code = '6'][position() &gt; 1])"/>
  ...
  <xsl:template match="
    *:datafield[not(@tag = '044')]
    [not(*:subfield[normalize-space(@code) = ''])]"
    mode="pass3">
    <datafield>
  ...
  <xsl:template match="*:datafield[@tag = '050' and matches(normalize-space(.), '^[A-Z]{3}$')]"
    mode="pass4"/>
  ...
  <xsl:template match="*:datafield[@tag = '099']/*:subfield[@code = 'v']" mode="pass4">
    <subfield>
         ...
         <xsl:apply-templates mode="pass4"/></subfield>
  </xsl:template>
  ...
  <xsl:template match="*:record" mode="phase2">
    <record>
      ...
      <xsl:for-each select="*:datafield[@tag = '090']">
        ...
        <xsl:if test="
          not(preceding-sibling::*:datafield
          [@tag = '090']
          [replace(normalize-space(.), '\s', '') = $thisValue]) 
          and 
          not(../*:datafield[@tag = '099']
          [replace(normalize-space(.), '\s', '') = $thisValue])">
          <xsl:apply-templates select="." mode="phase2"/>
        </xsl:if>
      </xsl:for-each>
  ...
  <xsl:template match="*:datafield[matches(@tag, '024')]/@ind1[not(matches(., '[0123478]'))]"
    mode="phase2">
    <xsl:attribute name="ind1">
      <xsl:text>8</xsl:text>
    </xsl:attribute>
            ...
            <xsl:choose>
              ...
              <xsl:when test="matches(., '([a-z]{3}[^a-z])+')">
                ...
                <xsl:analyze-string select="replace(normalize-space(.), '[^a-zA-Z]+$', '')"
                  ...
                  <xsl:non-matching-substring>
                    <subfield code="{$thisSubfield}"><!-- Fix common errors -->
                                 <xsl:value-of select="replace(replace(replace(replace(., 'jap', 'jpn'), 'ing', 'eng'), 'end', 'eng'), 'rur', 'rus')"/></subfield>
                  </xsl:non-matching-substring>
                </xsl:analyze-string>
              </xsl:when>
              <!-- Single code -->
              <xsl:otherwise>
                <subfield>
                           ...
                           <!-- Fix common errors -->
                           <xsl:value-of select="replace(replace(replace(replace(replace(normalize-space(.), '[^a-zA-Z]+$', ''), 'jap', 'jpn'), 'ing', 'eng'), 'end', 'eng'), 'rur', 'rus')"/></subfield>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:for-each>
        </xsl:copy>
  ...
  <xsl:template match="*:datafield[matches(@tag, '210|240')]/@ind1[not(matches(., '[01]'))]"
    mode="phase2">
    <xsl:attribute name="ind1">
      <xsl:text>1</xsl:text>
    </xsl:attribute>
        ...
        <xsl:when test="*:subfield[@code = 'u'][not(matches(., '^(mailto|ftp|telnet|https?)'))]">
          ...
          <xsl:for-each select="*:subfield[@code = 'u']">
            <xsl:if test="normalize-space(substring-before(*:subfield[@code = 'u'], ':'))">
              <subfield code="2">
                        <xsl:value-of select="substring-before(*:subfield[@code = 'u'][not(matches(., '^(mailto|ftp|telnet|https?)'))], ':')"/></subfield>
            </xsl:if>
          </xsl:for-each>
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template match="*:datafield[@tag = '242']/*:subfield[@code = 'y'][matches(., '\.$')]"
    mode="phase2">
    <subfield code="y">
         <xsl:value-of select="replace(., '\.+$', '')"/></subfield>
  </xsl:template>
      ...
      <xsl:choose>
        <xsl:when test="matches($subfields/*:subfield[@code = '6'], '/')">
          ...
          <xsl:choose>
            ...
            <xsl:when test="
              $linkingTag = '00' or 
              not(ancestor::*:record/*:datafield[matches
              (@tag, $linkingTag) and matches(*:subfield[@code = '6'], $linkedField)])">
        ...
        <xsl:when test="not(matches($subfields/*:subfield[@code = '6'], '/'))">
          ...
          <xsl:choose>
            ...
            <xsl:when test="
              $occurrenceNum = '00' or 
              not(ancestor::*:record/*:datafield[matches
              (@tag, $linkingTag) and matches(*:subfield[@code = '6'][1], $linkedField)])">
              <xsl:apply-templates select="@ind1" mode="phase2"/>
              <xsl:apply-templates select="@ind2" mode="phase2"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:copy>
      ...
      <xsl:choose>
        ...
        <xsl:when test="*:subfield[matches(., $pubLang)] and *:subfield[not(matches(., $pubLang))]">
          <subfield code="a">
                  <xsl:value-of select="*:subfield[matches(., $pubLang)][1]"/></subfield>
          <subfield code="h">
                  <xsl:value-of select="
                    replace(replace(replace(replace(replace(
                    *:subfield[not(matches(., $pubLang))][1], 
                    'ser', 'srp'), 
                    'cro', 'hrv'), 
                    'scs', 'srp'), 
                    'scc', 'srp'), 
  ...
  <xsl:template match="*:datafield[@tag = '041']/*:subfield[matches(@code, '[a-z]')]" mode="phase3">
    ...
    <subfield code="{$thisCode}">
         <xsl:value-of select="replace(replace(replace(replace(replace(replace(., 'ser', 'srp'), 'cro', 'hrv'), 'scs', 'srp'), 'scc', 'srp'), 'scr', 'srp'), 'fle', 'dut')"/></subfield>
  </xsl:template>
  ...
  <xsl:template
    ...
    <subfield code="{@code}"><!-- Fix length and common errors -->
         <xsl:value-of select="replace(substring(concat(lower-case(normalize-space(.)), '   '), 1, 3), 'fre', 'fr ')"/></subfield>
  </xsl:template>
